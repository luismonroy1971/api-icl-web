...
import {
  OpcionesUsuario
} from '../models/OpcionesUsuario.js';
import {
  Menu
} from '../models/Menu.js';
import {
  ...
  try {
    ...
    if (opcionesusuarios && opcionesusuarios.length > 0) {
      ...
      return res
        .status(400)
        .json({
          message: 'Debe proporcionar al menos una opción de usuario'
        });
    }
    ...
    const usuariosConOpcionesFormato = usuarios.map((usuario) => {
      const opcionesFormato = usuario.opcionesusuarios ?
        ...
        .map((opcion) => {
          return opcion.menu ?
            {
              value: opcion.id_menu,
              label: opcion.menu.nombre_menu
            } :
            null;
        })
...
export const actualizarUsuario = async (req, res) => {
  try {
    ...
    const {
      ...
      password,
      opcionesusuarios
    } = req.body; // Nuevos datos del usuario y opciones
