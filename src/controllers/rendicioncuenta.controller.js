  ...
  try {
      ...
      if (rendicionFile) {
          ...
          if (flag_adjunto === 'URL') {
              ...
              nuevaRendicion.url_documento = `${baseUrl}/documentos/rendiciones/${safeFileName}`;
              nuevaRendicion.contenido_documento = null;
          } else if (flag_adjunto === 'BIN') {
              nuevaRendicion.url_documento = null;
              nuevaRendicion.contenido_documento = await fs.readFile(rendicionFile.path);
          }
      }

      await nuevaRendicion.save();
      res.status(201).json({ mensaje: 'Rendición creada con éxito', nuevaRendicion });
  } catch (error) {
      console.error(error);
      return res.status(500).json({ mensaje: 'Error al crear rendición', error: error.message });
  }
};
  ...
  try {
      ...
      if (rendicionFile) {
          ...
          if (flag_adjunto === 'URL') {
              await fs.mkdir(documentosDir, { recursive: true });
              await fs.copyFile(rendicionFile.path, filePath);
              rendicion.url_documento = `${baseUrl}/documentos/rendiciones/${safeFileName}`;
              rendicion.contenido_documento = null;
              rendicion.flag_adjunto = 'URL';
          } else if (flag_adjunto === 'BIN') {
              rendicion.url_documento = null;
              rendicion.contenido_documento = await fs.readFile(rendicionFile.path);
